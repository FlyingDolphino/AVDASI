...
function [CSred,CS] = WingParameterisation(x,nSpan,PLOT)

run NACA_2418  % root aerofoil
...
Chord             = linspace(5,1.5,nSpan); % chord distribution along the span (m)
...
for i=1:nSpan
   ...
   CS(i).ZYProfile_Norm    = NACA2418*(1-CS(i).beta) + NACA2412*CS(i).beta;
